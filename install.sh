sudo pacman-key --recv-key 3056513887B78AEB --keyserver keyserver.ubuntu.com -y
sudo pacman-key --lsign-key 3056513887B78AEB -y
sudo pacman -U 'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-keyring.pkg.tar.zst' -y
sudo echo "[chaotic-aur]" >> /etc/pacman.conf
sudo echo "Include = /etc/pacman.d/chaotic-mirrorlist" >> /etc/pacman.conf

sudo pacman -Sy --needed xorg base-devel git wget neovim nitrogen ttf-font-awesome ttf-jetbrains-mono mpv feh htop fish nemo rofi pluma firefox noto-fonts-emoji nerd-fonts pulseaudio nordic-theme nordzy-icon-theme python-pip nodejs npm xf86-video-intel dunst -y
